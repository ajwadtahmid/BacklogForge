...
class _CompletedTabState extends ConsumerState<CompletedTab> {
  ...
  Future<void> _bulkMoveToBacklog(List<Game> allGames) async {
    ...
    if (mounted) {
      ...
      messenger.showSnackBar(
        SnackBar(content: Text('${selected.length} game${selected.length == 1 ? '' : 's'} moved to backlog')),
      );
    }
  }
  ...
  Future<void> _bulkMarkPlaying(List<Game> allGames) async {
    final selected = allGames.where((g) => _selectedIds.contains(g.id)).toList();
    final messenger = ScaffoldMessenger.of(context);
    await ref.read(gameActionsProvider).bulkMarkPlaying(
      selected,
  ...
  Future<void> _bulkStopReplaying(List<Game> allGames) async {
        ...
        Expanded(
          child: gamesFuture.when(
            ...
            data: (games) {
              ...
              if (filtered.isEmpty) {
                return Center(
                  child: Text(_query.isEmpty ? 'No games completed' : 'No results'),
                );
              }
      ...
      child: InkWell(
        ...
        child: Row(
          children: [
            ...
            Expanded(child: AbsorbPointer(child: GameCard(game: game, isInCompletedTab: true))),
          ],
        ),
      ),
    );
